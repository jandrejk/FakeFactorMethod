...
then
    ...
    hadd -f -n 4 ${prefix}/${channel}-NOMINAL_ntuple_Data.root ${prefix}/tmp/EGamma*.root 
    hadd -f -n 4 ${prefix}/${channel}-NOMINAL_ntuple_EMB.root ${prefix}/tmp/Embedding2018*ElTau*.root 
fi
...
then
    ...
    hadd -f -n 4 ${prefix}/${channel}-NOMINAL_ntuple_EMB.root ${prefix}/tmp/Embedding2018*MuTau*.root &
fi
...
then
    ...
    hadd -f -n 4 ${prefix}/${channel}-NOMINAL_ntuple_EMB.root ${prefix}/tmp/Embedding2018*TauTau*.root
fi


hadd -f -n 4 ${prefix}/${channel}-NOMINAL_ntuple_DY.root ${prefix}/tmp/DY*madgraph*.root ${prefix}/tmp/EWKZ2Jets*.root 
