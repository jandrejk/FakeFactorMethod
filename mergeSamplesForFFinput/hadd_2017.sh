...
then
    ...
    hadd -f -n 4 ${prefix}/${channel}-NOMINAL_ntuple_EMB.root ${prefix}/tmp/Embedding2017*ElTau*.root 
fi
...
then
    ...
    hadd -f -n 4 ${prefix}/${channel}-NOMINAL_ntuple_EMB.root ${prefix}/tmp/Embedding2017*MuTau*.root &
fi
...
then
    ...
    hadd -f -n 4 ${prefix}/${channel}-NOMINAL_ntuple_EMB.root ${prefix}/tmp/Embedding2017*TauTau*.root
fi
